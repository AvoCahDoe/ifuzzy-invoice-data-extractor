...
class OcrBuilder(BaseBuilder):
    ...
    def ocr_extraction(
        ...
        recognition_results: List[OCRResult] = []
        
        # If a callback is provided in config, use it
        progress_callback = self.config.get("progress_callback") if self.config else None

        for start_idx in range(0, total_pages, batch_size):
            end_idx = min(start_idx + batch_size, total_pages)
            batch_images = images[start_idx:end_idx]
            batch_polys = line_polygons[start_idx:end_idx]
            batch_texts = line_original_texts[start_idx:end_idx]
            
            batch_results = self.recognition_model(
                images=batch_images,
                task_names=[self.ocr_task_name] * len(batch_images),
        ...
        # Original single-call logic replaced by batch loop above
